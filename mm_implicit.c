...
int mm_init(void) {
  ...
  testAlign(temp);
  /* printf("Start2 %p: %li => %li, %li, 1: %i\n", start, gl(start), 
	 block_size(start), is_alloc(start), (gl(start)==1));
	 printf("Big block: %li, %li\n", block_size(start+2), is_alloc(start+2));*/
...
void mm_checkheap(int verbose) {
  ...
  if(verbose>=3){
	...
	printf("ERROR: misalign not found: %p/n", currentBlock);
      
      /* header and footer information is the same */
      if(gl(currentBlock)!=
	 gl(currentBlock+block_size(currentBlock)+1))
    ...
    printf("Checked all blocks\n");
  }
  else//don't look at all memory 
